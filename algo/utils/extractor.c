...
/*   Updated: 2024/03/25 13:07:18by isb3             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	// if (!not_in_extract(largest, (*stack_a)->value, n / 3))
	n /= 4;
	if (!not_in_extract(largest, (*stack_a)->value, n))
	{
		pb(stack_a, stack_b);
		if ((*stack_b)->next && (*stack_b)->value < (*stack_b)->next->value)
			sb(stack_b);
		if ((*stack_a) && ruler(*stack_a) > 20 && (*stack_a)->value < (*stack_b)->value)
